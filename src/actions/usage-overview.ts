...
import { renderUsageOverview, systemMetricProgress, type UsageOverviewFace, type UsageOverviewProviderFace } from "../render";
...
export class UsageOverview extends SingletonAction<UsageOverviewSettings> {
	...
	async refreshAll(): Promise<void> {
		await Promise.all([...this.actions].map(async (target) => {
			try {
				await this.#refresh(target, this.#settings.get(target.id) ?? {});
			} catch (err) {
	...
	async #tick(): Promise<void> {
		...
		for (const target of this.actions) {
			...
			if (now - (this.#lastRefresh.get(target.id) ?? 0) >= intervalMs) {
				await this.#refresh(target, settings);
			}
		}
	}
	...
	async #cycle(target: OverviewAction, settings: UsageOverviewSettings): Promise<void> {
		try {
			...
			this.#settings.set(target.id, nextSettings);
			await target.setSettings(nextSettings);
			await this.#refresh(target, nextSettings);
		} catch (err) {
			streamDeck.logger.error("failed to cycle usage overview mode", err);
		}
	}

	async #refresh(target: OverviewAction, settings: UsageOverviewSettings): Promise<void> {
		this.#lastRefresh.set(target.id, Date.now());
		const now = new Date();
		const [claude, codex] = await Promise.all([this.#readProvider("claude", now), this.#readProvider("codex", now)]);
